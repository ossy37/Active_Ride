...
public class LockOn2 : MonoBehaviour {
	...
	void Update () {
		...
		//カメラをターゲットに向ける
		Transform cameraParent2 = MainCamera2.transform.parent;
		Quaternion targetRotation2 = Quaternion.LookRotation (target.transform.position - cameraParent2.position);
		cameraParent2.localRotation = Quaternion.Slerp (cameraParent2.localRotation, targetRotation2, Time.deltaTime * 10);
		cameraParent2.localRotation = new Quaternion(cameraParent2.localRotation.x, 0, 0, cameraParent2.localRotation.w);
