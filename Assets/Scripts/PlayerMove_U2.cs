...
public class PlayerMove_U2 : MonoBehaviour {
	...
	void Update () {
		...
		*/
		...
		if (Input.GetButton ("Boost2")  && boostPoint > 1) {
		...
		if(Input.GetAxis ("Horizontal2") == 0){
			...
			targetSpeed.x *= Mathf.Sign(Input.GetAxis ("Horizontal2"));
		}
		...
		if(Input.GetAxis ("Vertical2") == 0){
			...
			targetSpeed.z *= Mathf.Sign(Input.GetAxis ("Vertical2"));
		}
		...
		//ジャンプキーによる上昇
		if (Input.GetButton ("Jump2")  && boostPoint > 1) {
		...
		MainCamera2.GetComponent<CameraMotionBlur> ().velocityScale = motionBlurValue;
